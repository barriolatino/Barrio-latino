...
| Visuels, montage, sous-titres, cover, export | `tools/factory.py build NN` | `output/<date>-<slug>/` |
| Contrôle qualité automatique + corrections | `tools/factory.py build` / `review` | `output/.../qa.json` |
...
- Le calendrier des 30 jours passe d'abord (chaque jour est relié à une idée par
  `topic_id`). Ensuite, `tools/factory.py next` et `tools/factory.py pick` choisissent dans la banque.
...
### Sélection (`tools/factory.py pick`)
...
### Nouvelles idées (`tools/factory.py combine`, commande `/idea`)
...
`topics.json` (`origine: "generee"`). `tools/factory.py bank` signale quand il reste
moins de 40 idées neuves.

### Apprentissage

Après publication : `tools/factory.py stats NN --vues … --likes … --commentaires …
...
vues) oriente les sélections suivantes. `tools/factory.py bank` montre les catégories
...
(`tools/factory.py bank`) avant de choisir un sujet.
