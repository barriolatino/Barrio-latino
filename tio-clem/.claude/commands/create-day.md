...
5. **Visuels et voix** : par défaut, cartes Tio Clem et voix enregistrée par Tio Clem. Si une scène gagne vraiment à une image réelle, cherche une image sous licence acceptée (voir `PLAYBOOK.md` §14) et enregistre-la avec `python3 tools/factory.py asset-add` ; ou marque la scène `"asset_type": "generated"` si `IMAGE_PROVIDER` est configuré. N'utilise jamais une image dont la licence n'est pas vérifiée.
6. **Vérification du script** : `python3 tools/factory.py lint NN` jusqu'à zéro erreur ; traite aussi les avertissements quand c'est possible.
7. **Rendu + contrôle** : `python3 tools/factory.py build NN`. Si le statut est `NEEDS_REVIEW`, corrige le JSON (ou la recherche) et relance jusqu'à `READY_TO_PUBLISH`. Relis toi-même orthographe, accords et naturel à l'oral : le contrôle automatique ne remplace pas une relecture.
8. Regarde la cover et 2 ou 3 images de la vidéo (extrais-les avec ffmpeg) pour vérifier le rendu.
9. Ne publie rien sur TikTok. Termine en listant : ce qui a été créé, les sources utilisées, les fichiers réellement produits (vérifie avec `ls`), et les points à vérifier par un humain.
