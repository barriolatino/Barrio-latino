...
2. Lis `tioclem/content/published.json` et l'idée liée dans `tioclem/content/topics.json` (`topic_id` du calendrier). Ne reprends ni un sujet, ni un hook, ni un angle déjà utilisé. Si le thème est déjà apparu, choisis un angle différent (voir les `angles_reserves` des recherches existantes). Respecte la rotation : jamais une 3e publication de suite dans la même catégorie. Si le jour dépasse le calendrier, prends l'idée donnée par `python3 tioclem/factory.py next`.
