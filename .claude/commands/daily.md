...
1. `python3 tioclem/factory.py next` : identifie le prochain jour non produit et son sujet (calendrier, puis banque d'idées une fois les 30 jours faits).
2. Suis `.claude/commands/create-day.md` pour ce jour : recherche, création, contrôle, export des fichiers.
3. Le jour passe à `READY_TO_PUBLISH` dans `tioclem/content/published.json` quand le contrôle qualité est entièrement vert.
